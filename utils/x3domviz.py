...
def generate_html_viz(cursor,tgtp, timestamp=None):
    ...
    cursor.execute('SELECT object_id, ST_AsX3D(bbox), ST_AsX3D(cbb), ST_AsX3D(tophsproj),'
    ...
    with open(tgtp, 'w') as outd:
        rpos = " ".join([str(coord) for coord in qres[-1][-4:-1]]) #robot coords for given timestamp/image
        print(rpos)
        # Write html heading
        ...
        <Shape><Sphere radius=".1"/><Appearance USE="DARK_GRAY"/></Shape></Transform> ' + '\n') % rpos)
        ...
        for i,r in enumerate(qres):
            ...
            if r[0]==timestamp+'_poly0': #draw all boxes/hs only for one for readability
                outd.write('<shape><appearance><material></material></appearance>' + '\n')
                outd.write(r[1].replace('FaceSet','LineSet') + '\n')
                outd.write('</shape>' + '\n')
                #     # # fronths are red
            ...
            #    outd.write('</shape>' + '\n')
            #     # lefths are red
                outd.write('<shape><appearance><material emissiveColor="0.5 0. 0.0"></material></appearance>' + '\n')
...
def main():
    ...
    generate_html_viz(cur,output_p, timestamp='2020-05-15-11-12-27_125515')
